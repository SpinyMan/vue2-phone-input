    ...
    <div class="intl-phone-input allow-dropdown" v-on:mouseleave="hideList">
        <div class="flag-container" v-on:click.stop="toggeList">
            ...
            <ul class="country-list" v-if="isVisiblePanel">
                ...
                <li v-for="item in itemsData.otherItems" v-bind:class="`country${item.code === countryCode ? ' highlight' : ''}`" v-on:click.stop="setCode(item.code)">
                    <div class="flag-box">
                        <div v-bind:class="`iti-flag ${item.code}`"></div>
                    </div>
                    <span class="country-name">{{ item.name }}</span>
        ...
        <input type="text" autocomplete="off" v-bind:name="name" v-on:input="handleChangePhoneNumber" v-bind:value="phone.number" @keyup="changeCountry" v-bind:placeholder="intlData.example"/>
    </div>
</template>
...
<script>
    ...
    const phonesData = {
        ...
        by: {code: 'by', name: 'Belarus (Беларусь)', dialCode: 375, example: '(29) 765-43-21'},
        ...
        ua: {code: 'ua', name: 'Ukraine (Україна)', dialCode: 380, example: '39 123 4567'},
        ae: {
            code: 'ae',
